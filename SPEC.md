...
| `x` | T9 | Add TOML linter coverage test to verify `taplo` is exercised on `.gitleaks.toml` and `.rtk/filters.toml` |
...
11. **`lefthook.yml` references unbuilt `lefthook-markdownlint` wrapper.** The refresh added a `markdownlint` hook running `lefthook-markdownlint`, but only `lefthook-markdownlint-agentic` was wired into the flake. `command -v lefthook-markdownlint` failed and CI `build-linux` aborted with `timeout: failed to run command 'lefthook-markdownlint': No such file or directory` (`exit status 127`). Fixed by adding the `nix-lefthook-markdownlint` flake input and its wrapper (plus the `is-markdown-agentic` helper it depends on) in `nix/lefthook-wrappers-for.nix`, with a 1-to-1 bats test.
12. **`lefthook.yml` `exclude` used regex strings that lefthook 2.x silently ignores.** The `markdownlint`/`markdownlint-agentic` hooks used string-regex excludes (`exclude: "^(README|CHANGELOG|SPEC|CONTRIBUTING)\.md$"`). lefthook 2.x only honours a *list of globs* for `exclude`; a bare regex string is accepted but never applied, so `SPEC.md` (and README/CHANGELOG) leaked into `markdownlint-agentic` and failed `MD013/line-length` (Actual 631 > Expected 300), aborting CI `build-linux` with `exit status 1`. Fixed by rewriting both excludes as glob lists (`- "SPEC.md"`, `- ".claude/skills/**"`, …), restoring the intended exclusion, with a regression test in `tests/unit/lefthook_yml.bats`.
13. **Guardrail fidelity failed after migration to `set-and-setting`.** `flake.nix` declared only the `base` and `nix` lefthook fragments and `.gitignore` excluded the materialized `lefthook.yml`, while repository detection required `base nix shell ascii markdown yaml`. Fixed by declaring all detected fragments and tracking the canonical materialized `lefthook.yml`.
14. **Guardrail coherence ran without fragment wrappers on `PATH`.** The `confirm` app invoked the coherence check with only generic Unix tools in its runtime closure, even though `lefthook.yml` referenced commands supplied by the selected fragments. Fixed by adding the materialization's fragment packages to the app's `runtimeInputs`, matching the dev shell environment.
15. **The migrated repository omitted the file-size policy.** The pinned `file-size-check` runs hermetically and requires the repository-specific `config/lefthook/file_size_limits.yml`, but migration left the seed file absent, so `nix flake check` failed after confirmation. Fixed by committing the standard per-extension limits used by the selected base fragment.
16. **The migrated `confirm` app embedded its launcher shell in `flake.nix`.** Once the full pinned checks ran, `nix-no-embedded-shell` rejected the inline environment setup. Fixed by extracting the launcher to `scripts/confirm.sh` and using placeholder substitution for Nix store paths.
17. **`set-and-setting` flake update removed `lib` output.** `nix flake update` pinned `set-and-setting` to a version (d2fa92cc) that no longer exposes a top-level `lib` attribute; `flake.nix` referenced `set-and-setting.lib.mkSetting`, `set-and-setting.lib.materializationFor`, etc., causing `error: attribute 'lib' missing` during `nix flake check`. Fixed by resolving `lib` through `set-and-setting.inputs.set-and-setting`, the inner pinned version that still exports the API.
18. **The guardrail manifest check rejected the inline outputs implementation.** The `flake-manifest` guardrail requires `flake.nix` to delegate `outputs` to a separate module, but `flake.nix` defined a large `let` body inline, causing `outputs body must not be a let expression`. Fixed by moving the outputs implementation to `nix/outputs.nix` and retaining the flake as a thin delegating manifest.
19. **Guardrail fidelity failed because the materialization omitted the detected `actions` fragment and ignored the generated `lefthook.yml`.** The guardrail expected `base actions nix shell ascii markdown yaml`, but the flake declared only the other six fragments and `.gitignore` excluded the materialized file. Fixed by adding the `actions` fragment and tracking the canonical generated `lefthook.yml`.
20. **The extracted outputs module was not formatted with the repository's pinned `nixfmt`.** The guardrails' `nixfmt` check rejected the long `actionlint` wrapper expression in `nix/outputs.nix`. Fixed by applying nixfmt's required multiline wrapping.
21. **Guardrail rejected scalar `glob` values in `lefthook.yml`.** The migrated guardrail schema requires glob matchers to be lists, but all local commands used scalar strings (including brace-expanded workflow globs), causing `expected a list but found a string`. Fixed by converting every `glob` to a list and expanding the workflow patterns into separate YAML and YAML-with-extension entries.
22. **Pinned `set-and-setting` actionlint check passed a scalar regex to nixpkgs source filtering.** Enabling the `actions` fragment made `nix flake check` fail with `expected a list but found a string: "^.github/workflows/.*"`. Fixed by retaining the actions check but supplying the workflow regex as a list in the repository's compatibility check until the dependency updates its API call.
23. **The linter-coverage guardrail required an exemption ledger.** The refreshed `set-and-setting` checks failed with `linter-coverage: missing config/linter-coverage-exemptions.yml` because the newly required ledger was not present in the repository source. Fixed by adding the required ledger for intentionally exempt file classes with tracked ticket IDs.
24. **The generated dev shell omitted the repository's `lefthook` executable.** It passed only materialized fragment packages to `mkDevShells`, so CI's isolated hook-install/run step ended with `lefthook: command not found` (`exit 127`). Fixed by adding the pinned `lefthook` package to the dev shell's base packages.
25. **The pinned `nix-lefthook-bats-failures-only` wrapper failed its upstream Bats validation.** The guardrails check used the stale `bf0eb2d` source revision, whose package check produced multiple `teardown_file failed` errors. Fixed by updating only that flake input to the upstream pin-refresh revision `1a5d442`.
26. **The pinned CI action requested a missing `devShells.<system>.ci` output.** The flake exposed only the shells generated by `mkDevShells`, so `nix develop .#ci` failed before any checks ran. Fixed by adding a dedicated CI shell containing the materialized tools, pinned `lefthook`, and CI environment defaults.
27. **The isolated CI dev shell omitted the `nix` executable.** The pinned CI action runs lefthook with `nix develop --ignore-environment`; the `nix flake check` hook then could not find `nix` on the inner shell's `PATH` and exited 127. Fixed by adding `pkgs.nix` to the dev shell package set.
